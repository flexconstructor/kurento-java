...
public class JsonRpcClientNettyWebSocket extends AbstractJsonRpcClientWebSocket {
    ...
    @Override
    protected Mono<Void> sendTextMessage(Mono<String> jsonMessage) {
        return this.nettyOutbound.sendString(jsonMessage).then();
    }
    ...
    protected Mono<Void> connectNativeClient() {
        ...
        if (ssl) {
            this.connectionMono = TcpClient.create().host(host).port(port)
                    .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, this.connectionTimeout)
                    .secure(sslContextSpec -> {
                        sslContextSpec.sslContext(sslCtx);
                    }).connect(b);
        } else {
            this.connectionMono = TcpClient.create().host(host).port(port).handle(this::handleConnection).connect(b);
        }
        return this.connectionMono.flatMap(DisposableChannel::onDispose);
    }
    ...
    @Override
    public Mono<Void> closeNativeClient() {
        Mono<Void> result = closeChannel();
    ...
    private Mono<Void> closeChannel() {
        return this.connectionMono.flatMap(cnt -> {
            cnt.dispose();
            return cnt.onDispose();
        });
    }
