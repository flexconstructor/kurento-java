...
import static org.kurento.jsonrpc.JsonUtils.*;
...
public abstract class AbstractReactiveJsonRpcClientWebSocket extends JsonRpcClient {

  private static final int CONNECTION_LOCK_TIMEOUT = 25000;

  private static Logger log = LoggerFactory.getLogger(AbstractReactiveJsonRpcClientWebSocket.class);
  ...
  public AbstractReactiveJsonRpcClientWebSocket(String url,
      ...
      synchronized (executorsLock) {
        ...
        if (reqResEventExec == null || reqResEventExec.isShutdown()
          ...
          reqResEventExec = Executors.newCachedThreadPool(
              ThreadFactoryCreator.create("AbstractReactiveJsonRpcClientWebSocket-reqResEventExec"));
        }

        if (disconnectExec == null || disconnectExec.isShutdown()
            || disconnectExec.isTerminated()) {
          disconnectExec = Executors.newScheduledThreadPool(1,
              ThreadFactoryCreator.create("AbstractReactiveJsonRpcClientWebSocket-disconnectExec"));
        }
      }
    }
  }
  ...
  protected Mono<Void> internalConnectIfNecessary(boolean shutdownReconnectThread){
    ...
    if (!isNativeClientConnected()) {
      ...
      if (isClosedByUser()) {
        return Mono.error( new JsonRpcClientClosedException(
      ...
      return connectNativeClient().then(this.updateSession()).onErrorResume(error
       -> {
        ...
        if (error instanceof TimeoutException) {
          ...
          if (retryingIfTimeoutToConnect) {
            ...
            return internalConnectIfNecessary(shutdownReconnectThread);
          }
        }else{
        ...
        return Mono.error(new JsonRpcException(exceptionMessage, error));
      });
  ...
  boolean executeConnectProtocol() {
    try {
      rsHelper.sendRequest(METHOD_CONNECT, String.class);
      ...
      throw new JsonRpcException(label + " Exception executing reconnect protocol", ex);
    }
  }
  ...
  protected abstract void sendTextMessage(Message jsonMessage);
