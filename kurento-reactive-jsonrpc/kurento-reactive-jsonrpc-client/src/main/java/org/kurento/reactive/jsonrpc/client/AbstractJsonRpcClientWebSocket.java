...
import org.kurento.reactive.jsonrpc.JsonRpcClientClosedException;
import org.kurento.reactive.jsonrpc.JsonRpcErrorException;
import org.kurento.reactive.jsonrpc.JsonRpcException;
import org.kurento.reactive.jsonrpc.internal.JsonRpcConstants;
import org.kurento.reactive.jsonrpc.internal.client.ClientSession;
import org.kurento.reactive.jsonrpc.internal.client.TransactionImpl.ResponseSender;
import org.kurento.reactive.jsonrpc.message.Message;
import org.kurento.reactive.jsonrpc.message.MessageUtils;
...
import static org.kurento.reactive.jsonrpc.JsonUtils.*;
import static org.kurento.reactive.jsonrpc.internal.JsonRpcConstants.METHOD_CONNECT;
import static org.kurento.reactive.jsonrpc.internal.JsonRpcConstants.METHOD_PING;
...
public abstract class AbstractJsonRpcClientWebSocket extends JsonRpcClient {
  ...
  protected <P, R> Response<R> internalSendRequestWebSocket(Request<P> request,
   ...
   /*connectIfNecessary(false);
    ...
    @Override
    public Mono<Void> close(@Nullable String closeMessage) {
        ...
        if (closeMessage != null) {
            return this.sendNotification(JsonRpcConstants.METHOD_CLOSE, Mono.just(closeMessage))
                    .onErrorMap(ex -> {
                        log.warn("{} Exception sending close message. {}:{}", label, ex.getClass().getName(),
                                ex.getMessage());
                        return ex;
                    })
                    .and(closeClient("Session closed by JsonRpcClientWebsocket user", false, true));
        }
        return closeClient("Session closed by JsonRpcClientWebsocket user", false, true);
    }


    protected Mono<Void> closeClient(String reason, boolean reconnecting, boolean shutdownReconnectThread) {
    ...
    if (disconnectExec != null && shutdownReconnectThread) {
      ...
      /*  if (heartbeating) {
      disableHeartbeat();
    }*/ TODO:// disable heartbeating
        return closeNativeClient();

    }

    private Mono<Void> notifyDisconnection(String reason, boolean connectedBefore) {
    ...
    protected Mono<Void> handleResponseFromServer(JsonObject message) {
    ...
    @Override
    protected Mono<Void> closeWithReconnection() {
    ...
    @Override
    public Mono<Void> connect() throws IOException {
    ...
    void configureResponseSender() {
        rs = new ResponseSender() {
            @Override
            public Mono<Void> sendResponse(Message message) throws IOException {
            ...
            @Override
            public Mono<Void> sendPingResponse(Message message) throws IOException {
                String jsonMessage = message.toString();
    ...
    protected abstract Mono<Void> sendTextMessage(Mono<String> jsonMessage);

    protected abstract Mono<Void> closeNativeClient();


    protected abstract Mono<Void> connectNativeClient();
